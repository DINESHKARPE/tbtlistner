...
function Device(adapter, connection, gpsServer) {
  ...
  this.on("data", function (data) {
    let msgParts = _this.adapter.parse_data(data);
  ...
  this.make_action = function (action, msgParts) {
    ...
    //     return false;
    // }
    // dataAccess.insertLivePoi(msgParts, function (content) {});
    switch (action) {
      case "login_request":
  ...
  this.login_authorized = function (val, msgParts) {
    if (val) {
      this.do_log(
        "Device " + _this.getUID() + " has been authorized. Welcome!"
      );
      this.loged = true;
      this.adapter.authorize(msgParts);
    } else {
      this.do_log(
        "Device " + _this.getUID() + " not authorized. Login request rejected"
      );
    }
  };
  ...
  this.ping = function (msgParts) {
    let gpsData = this.adapter.get_ping_data(msgParts);
    if (gpsData === false) {
      //Something bad happened
  ...
  /****************************************
     Start GPS Tracking
     ****************************************/
  this.start = function (msgParts) {
    //We pass the message parts to the adapter and they have to say wich type of alarm it is.
    var alarmData = _this.adapter.get_start_data(msgParts);
    /* Alarm data must return an object with at least:
         alarm_type: object with this format:
         {'code':'sos_alarm','msg':'SOS Alarm activated by the driver'}
         */
    _this.emit("START", alarmData.data, alarmData, msgParts);
  };
  /****************************************
     Stop GPS Tracking
     ****************************************/
  this.stop = function (msgParts) {
    //We pass the message parts to the adapter and they have to say wich type of alarm it is.
    var alarmData = _this.adapter.get_stop_data(msgParts);
    /* Alarm data must return an object with at least:
         alarm_type: object with this format:
         {'code':'sos_alarm','msg':'SOS Alarm activated by the driver'}
         */
    _this.emit("STOP", "STOP", alarmData, msgParts);
  };
  /****************************************
     SET REFRESH TIME
     ****************************************/
